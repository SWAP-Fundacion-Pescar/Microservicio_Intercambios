...
class ExchangeQuery implements IExchangeQuery
{
    async getExchangeById(exchangeId: string): Promise<IExchangeDocument> {
        ...
        if(!retrievedExchange) throw new NotFoundException('No se ha encontrado el intercambio buscado');
    ...
    async getExchangeByUserId(userId: string): Promise<Array<IExchangeDocument>> {
        ...
        if(!retrievedUsers) throw new NotFoundException('Este usuario no tiene intercambios activos');
    ...
    async getExchangeByClotheId(clotheId: string): Promise<IExchangeDocument> {
        ...
        if(!retrievedClothe) throw new NotFoundException('Ha ocurrido un error. La prenda buscada no está disponible');
